...
void ALMAHealthPickup::NotifyActorBeginOverlap(AActor *OtherActor) {
  ...
  const auto Charcter = Cast<ALMADefaultCharacter1>(OtherActor);
...
bool ALMAHealthPickup::GivePickup(ALMADefaultCharacter1 *Character) {
...
void ALMAHealthPickup::PickupWasTaken() {
  ...
  GetWorldTimerManager().SetTimer(
      RespawnTimerHandle, this, &ALMAHealthPickup::RespawnPickup, RespawnTime);
}
